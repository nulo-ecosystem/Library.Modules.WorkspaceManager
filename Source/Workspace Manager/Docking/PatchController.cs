...
namespace Nulo.Modules.WorkspaceManager.Docking {
    ...
    public static class PatchController {
        ...
        public static void Reset() {
            EnableAll = _highDpi = _memoryLeakFix = _nestedDisposalFix = _focusLostFix = _contentOrderFix = _fontInheritanceFix = _activeXFix = _displayingPaneFix = _activeControlFix = _floatSplitterFix = _activateOnDockFix = _selectClosestOnClose = null;
        }
        ...
        public static bool? EnableHighDpi {
            get {
...
#if NET35 || NET40
                var section = ConfigurationManager.GetSection("WorkspaceManager") as PatchSection;
                if (section != null) {
                ...
                {
                    var key = Registry.CurrentUser.OpenSubKey(@"Software\WorkspaceManager");
                    if(key != null) {
                ...
                {
                    var key = Registry.LocalMachine.OpenSubKey(@"Software\WorkspaceManager");
                    if(key != null) {
                        var pair = key.GetValue("EnableHighDpi");
                        if(pair != null) {
                            if(bool.TryParse(pair.ToString(), out bool enable)) {
        ...
        #endregion Copy this section to create new option, and then comment it to show what needs to be modified.
