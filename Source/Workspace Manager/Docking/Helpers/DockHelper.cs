namespace Nulo.Modules.WorkspaceManager.Docking {
    ...
    public static class DockHelper {
        ...
        public static DockPane PaneAtPoint(Point pt, DockPanel dockPanel) {
            if(!Win32Helper.IsRunningOnMono)
                for(Control control = Win32Helper.ControlAtPoint(pt); control != null; control = control.Parent) {
                    if(control is IDockContent content && content.DockHandler.DockPanel == dockPanel) { return content.DockHandler.Pane; }
                    if(control is DockPane pane && pane.DockPanel == dockPanel) { return pane; }
                }
        ...
        public static FloatWindow FloatWindowAtPoint(Point pt, DockPanel dockPanel) {
            if(!Win32Helper.IsRunningOnMono)
                for(Control control = Win32Helper.ControlAtPoint(pt); control != null; control = control.Parent) {
                    if(control is FloatWindow floatWindow && floatWindow.DockPanel == dockPanel) { return floatWindow; }
                }
