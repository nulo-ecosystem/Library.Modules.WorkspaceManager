namespace Nulo.Modules.WorkspaceManager.Docking {
    ...
    public partial class DockPane : UserControl, IDockDragSource {
        ...
        private void InternalConstruct(IDockContent content, DockState dockState, bool flagBounds, Rectangle floatWindowBounds, DockPane prevPane, DockAlignment alignment, double proportion, bool show) {
            if(dockState == DockState.Hidden || dockState == DockState.Unknown) { throw new ArgumentException(Strings.DockPane_SetDockState_InvalidState); }
            if(content == null) { throw new ArgumentNullException(Strings.DockPane_Constructor_NullContent); }
            if(content.DockHandler.DockPanel == null) { throw new ArgumentException(Strings.DockPane_Constructor_NullDockPanel); }
        ...
        protected override void Dispose(bool disposing) {
            if(disposing) {
                ...
                // https://github.com/WorkspaceManager/WorkspaceManager/issues/16
        ...
        public virtual IDockContent ActiveContent {
            ...
            set {
                ...
                if(value != null) {
                    if(!DisplayingContents.Contains(value)) { throw (new InvalidOperationException(Strings.DockPane_ActiveContent_InvalidValue)); }
                } else {
                    if(DisplayingContents.Count != 0) { throw (new InvalidOperationException(Strings.DockPane_ActiveContent_InvalidValue)); }
                }

                IDockContent oldValue = m_activeContent;

                if(DockPanel.ActiveAutoHideContent == oldValue) { DockPanel.ActiveAutoHideContent = null; }
                ...
                if(DockPanel.DocumentStyle == DocumentStyle.DockingMdi && DockState == DockState.Document) {
                    ...
                    if(oldValue != null && DisplayingContents.Contains(oldValue)) { oldValue.DockHandler.SetVisible(); }
                    if(IsActivated && m_activeContent != null) { m_activeContent.DockHandler.Activate(); }
                }
                ...
                if(m_activeContent != null) { TabStripControl.EnsureTabVisible(m_activeContent); }
            }
        }
        ...
        private object TabPageContextMenu {
            get {
                ...
                if(content.DockHandler.TabPageContextMenuStrip != null) { return content.DockHandler.TabPageContextMenuStrip; }
#if NET35 || NET40
                else if (content.DockHandler.TabPageContextMenu != null) { return content.DockHandler.TabPageContextMenu; }
#endif
                else { return null; }
            }
        }
        ...
        internal Rectangle TabStripRectangle {
            get {
                return Appearance == AppearanceStyle.ToolWindow ? TabStripRectangle_ToolWindow : TabStripRectangle_Document;
            }
        }

        private Rectangle TabStripRectangle_ToolWindow {
            get {
                if(DisplayingContents.Count <= 1 || IsAutoHide) { return Rectangle.Empty; }
                Rectangle rectWindow = DisplayingRectangle;
                int width = rectWindow.Width;
                ...
                Rectangle rectCaption = CaptionRectangle;
                if(rectCaption.Contains(x, y)) { y = rectCaption.Y + rectCaption.Height; }
                return new Rectangle(x, y, width, height);
            }
        }
        ...
        private Rectangle TabStripRectangle_Document {
            get {
                if(DisplayingContents.Count == 0) { return Rectangle.Empty; }
                if(DisplayingContents.Count == 1 && DockPanel.DocumentStyle == DocumentStyle.DockingSdi) { return Rectangle.Empty; }
                Rectangle rectWindow = DisplayingRectangle;
                int x = rectWindow.X;
                int width = rectWindow.Width;
                int height = TabStripControl.MeasureHeight();
                int y = DockPanel.DocumentTabStripLocation == DocumentTabStripLocation.Bottom ? rectWindow.Height - height : rectWindow.Y;
                return new Rectangle(x, y, width, height);
            }
        }
        ...
        private bool HasCaption {
            get {
                return DockState != DockState.Document && DockState != DockState.Hidden && DockState != DockState.Unknown && (DockState != DockState.Float || FloatWindow.VisibleNestedPanes.Count > 1);
            }
        }
        ...
        internal void SetIsActivated(bool value) {
            ...
            m_isActivated = value;
            if(DockState != DockState.Document) { RefreshChanges(false); }
        ...
        internal void SetIsActiveDocumentPane(bool value) {
            if(m_isActiveDocumentPane == value) { return; }
            m_isActiveDocumentPane = value;
            if(DockState == DockState.Document) { RefreshChanges(); }
        ...
        public bool IsDockStateValid(DockState dockState) {
            foreach(IDockContent content in Contents) {
                if(!content.DockHandler.IsDockStateValid(dockState)) { return false; }
            }
            return true;
        }
        ...
        internal void CloseContent(IDockContent content) {
            ...
            if(!content.DockHandler.CloseButton) { return; }
        ...
        public void SetContentIndex(IDockContent content, int index) {
            ...
            if(oldIndex == Contents.Count - 1 && index == -1) { return; }
        ...
        internal void TestDrop(IDockDragSource dragSource, DockOutlineBase dockOutline) {
            if(!dragSource.CanDockTo(this)) { return; }
        ...
        internal void ValidateActiveContent() {
            ...
            if(DisplayingContents.IndexOf(ActiveContent) >= 0) { return; }
        ...
        public INestedPanesContainer NestedPanesContainer {
            get {
                return NestedDockingStatus.NestedPanes == null ? null : NestedDockingStatus.NestedPanes.Container;
            }
        }
        ...
        private void RefreshStateChange(INestedPanesContainer oldContainer, DockState oldDockState) {
            if(IsRefreshStateChangeSuspended) { return; }
            ...
            if(DockHelper.IsDockStateAutoHide(oldDockState)) { DockPanel.RefreshActiveAutoHideContent(); }
            if(NestedPanesContainer.DockState == DockState) { ((Control)NestedPanesContainer).PerformLayout(); }
            if(DockHelper.IsDockStateAutoHide(DockState)) { DockPanel.RefreshActiveAutoHideContent(); }
            ...
            contentFocused?.DockHandler.Activate();
            DockPanel.ResumeLayout(true, true);
            if(oldDockState != DockState) { OnDockStateChanged(EventArgs.Empty); }
        }
        ...
        public DockPane DockTo(INestedPanesContainer container) {
            if(container == null) { throw new InvalidOperationException(Strings.DockPane_DockTo_NullContainer); }
            DockAlignment alignment = container.DockState == DockState.DockLeft || container.DockState == DockState.DockRight ? DockAlignment.Bottom : DockAlignment.Right;
        ...
        public DockPane DockTo(INestedPanesContainer container, DockPane previousPane, DockAlignment alignment, double proportion) {
            ...
            pane = container.IsFloat ? DockPanel.Theme.Extender.DockPaneFactory.CreateDockPane(DockPanel.DummyContent, (FloatWindow)container, true) : DockPanel.Theme.Extender.DockPaneFactory.CreateDockPane(DockPanel.DummyContent, container.DockState, true);
        ...
        private void InternalAddToDockList(INestedPanesContainer container, DockPane prevPane, DockAlignment alignment, double proportion) {
            if(container.DockState == DockState.Float != IsFloat) { throw new InvalidOperationException(Strings.DockPane_DockTo_InvalidContainer); }
            int count = container.NestedPanes.Count;
            if(container.NestedPanes.Contains(this)) { count--; }
            if(prevPane == null && count > 0) { throw new InvalidOperationException(Strings.DockPane_DockTo_NullPrevPane); }
            if(prevPane != null && !container.NestedPanes.Contains(prevPane)) { throw new InvalidOperationException(Strings.DockPane_DockTo_NoPrevPane); }
            if(prevPane == this) { throw new InvalidOperationException(Strings.DockPane_DockTo_SelfPrevPane); }
            INestedPanesContainer oldContainer = NestedPanesContainer;
        ...
        public void SetNestedDockingProportion(double proportion) {
            ...
            if(NestedPanesContainer != null) { ((Control)NestedPanesContainer).PerformLayout(); }
        }
        ...
        private DockPane GetFloatPaneFromContents() {
            ...
            for(int i = 0; i < DisplayingContents.Count; i++) {
                IDockContent content = DisplayingContents[i];
                if(!content.DockHandler.IsDockStateValid(DockState.Float)) { continue; }
        ...
        private IDockContent GetFirstContent(DockState dockState) {
            for(int i = 0; i < DisplayingContents.Count; i++) {
                IDockContent content = DisplayingContents[i];
                if(content.DockHandler.IsDockStateValid(dockState)) { return content; }
            }
            return null;
        }
        ...
        Rectangle IDockDragSource.BeginDrag(Point ptMouse) {
            ...
            size = DockState == DockState.Float || floatPane == null || floatPane.FloatWindow.NestedPanes.Count != 1 ? DockPanel.DefaultFloatWindowSize : floatPane.FloatWindow.Size;
