...
namespace Nulo.Modules.WorkspaceManager.Docking {
    ...
    partial class DockPanel {
        ...
        public class AutoHideWindowControl : Panel, ISplitterHost {
            ...
            private const int ANIMATE_TIME = 50;    // in mini-seconds

            #endregion consts

            private readonly Timer m_timerMouseTrack;
            protected SplitterBase MSplitter { get; private set; }
            ...
            public AutoHideWindowControl(DockPanel dockPanel) {
                ...
                Visible = false;
                MSplitter = DockPanel.Theme.Extender.WindowSplitterControlFactory.CreateSplitterControl(this);
            ...
            public IDockContent ActiveContent {
                ...
                set {
                    if(value == m_activeContent) { return; }
                    ...
                    SetActivePane();
                    if(ActivePane != null) { ActivePane.ActiveContent = m_activeContent; }
                    if(m_activeContent != null) { AnimateWindow(true); }
            ...
            internal bool FlagDragging {
                ...
                set {
                    if(m_flagDragging == value) { return; }
            ...
            private void AnimateWindow(bool show) {
                ...
                int speedFactor = 1;
                int totalPixels = (rectSource.Width != rectTarget.Width) ? Math.Abs(rectSource.Width - rectTarget.Width) : Math.Abs(rectSource.Height - rectTarget.Height);
                ...
                while(rectSource != rectTarget) {
                    ...
                    if(Math.Sign(rectTarget.X - rectSource.X) != Math.Sign(dxLoc)) { rectSource.X = rectTarget.X; }
                    if(Math.Sign(rectTarget.Y - rectSource.Y) != Math.Sign(dyLoc)) { rectSource.Y = rectTarget.Y; }
                    if(Math.Sign(rectTarget.Width - rectSource.Width) != Math.Sign(dWidth)) { rectSource.Width = rectTarget.Width; }
                    if(Math.Sign(rectTarget.Height - rectSource.Height) != Math.Sign(dHeight)) { rectSource.Height = rectTarget.Height; }
            ...
            private Rectangle GetRectangle(bool show) {
                if(DockState == DockState.Unknown) { return Rectangle.Empty; }
        ...
        internal Rectangle AutoHideWindowRectangle {
            get {
                ...
                if(ActiveAutoHideContent == null) { return Rectangle.Empty; }
                if(Parent == null) { return Rectangle.Empty; }

                Rectangle rect = Rectangle.Empty;
                double autoHideSize = ActiveAutoHideContent.DockHandler.AutoHidePortion;
                if(state == DockState.DockLeftAutoHide) {
                    if(autoHideSize < 1) { autoHideSize = rectDockArea.Width * autoHideSize; }
                    if(autoHideSize > rectDockArea.Width - MeasurePane.MinSize) { autoHideSize = rectDockArea.Width - MeasurePane.MinSize; }
                    ...
                    if(autoHideSize < 1) { autoHideSize = rectDockArea.Height * autoHideSize; }
                    if(autoHideSize > rectDockArea.Height - MeasurePane.MinSize) { autoHideSize = rectDockArea.Height - MeasurePane.MinSize; }
        ...
        internal Rectangle GetAutoHideWindowBounds(Rectangle rectAutoHideWindow) {
            return DocumentStyle == DocumentStyle.SystemMdi || DocumentStyle == DocumentStyle.DockingMdi ? (Parent == null) ? Rectangle.Empty : Parent.RectangleToClient(RectangleToScreen(rectAutoHideWindow)) : rectAutoHideWindow;
        }
