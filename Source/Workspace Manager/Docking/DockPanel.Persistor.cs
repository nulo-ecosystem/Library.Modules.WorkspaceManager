        ...
        private static class Persistor {
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn) {
                ...
                for(int i = 0; i < countOfContents; i++) {
                    ...
                    if(xmlIn.Name != "Content" || id != i) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
            ...
            private static PaneStruct[] LoadPanes(XmlTextReader xmlIn) {
                ...
                for(int i = 0; i < countOfPanes; i++) {
                    int id = Convert.ToInt32(xmlIn.GetAttribute("ID"), CultureInfo.InvariantCulture);
                    if(xmlIn.Name != "Pane" || id != i) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    ...
                    MoveToNextElement(xmlIn);
                    if(xmlIn.Name != "Contents") { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    ...
                    for(int j = 0; j < countOfPaneContents; j++) {
                        ...
                        if(xmlIn.Name != "Content" || id2 != j) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
            ...
            private static DockWindowStruct[] LoadDockWindows(XmlTextReader xmlIn, DockPanel dockPanel) {
                ...
                for(int i = 0; i < countOfDockWindows; i++) {
                    int id = Convert.ToInt32(xmlIn.GetAttribute("ID"), CultureInfo.InvariantCulture);
                    if(xmlIn.Name != "DockWindow" || id != i) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    ...
                    MoveToNextElement(xmlIn);
                    if(xmlIn.Name != "DockList" && xmlIn.Name != "NestedPanes") { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    ...
                    for(int j = 0; j < countOfNestedPanes; j++) {
                        int id2 = Convert.ToInt32(xmlIn.GetAttribute("ID"), CultureInfo.InvariantCulture);
                        if(xmlIn.Name != "Pane" || id2 != j) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
            ...
            private static FloatWindowStruct[] LoadFloatWindows(XmlTextReader xmlIn) {
                ...
                for(int i = 0; i < countOfFloatWindows; i++) {
                    int id = Convert.ToInt32(xmlIn.GetAttribute("ID"), CultureInfo.InvariantCulture);
                    if(xmlIn.Name != "FloatWindow" || id != i) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
            ...
            private static bool IsFormatVersionValid(string formatVersion) {
                ...
                foreach(string s in CompatibleConfigFileVersions) {
                    if(s == formatVersion) { return true; }
                }
                return false;
            }
            ...
            public static void LoadFromXml(DockPanel dockPanel, string xmlContent, DeserializeDockContent deserializeContent) {
                ...
                using(var xmlIn = new XmlTextReader(new StringReader(xmlContent)) { WhitespaceHandling = WhitespaceHandling.None }) {
                    ...
                    while(!xmlIn.Name.Equals("DockPanel")) {
                        if(!MoveToNextElement(xmlIn)) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    }

                    string formatVersion = xmlIn.GetAttribute("FormatVersion");
                    if(!IsFormatVersionValid(formatVersion)) { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidFormatVersion); }
                    ...
                    // Load Panes
                    if(xmlIn.Name != "Panes") { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    panes = LoadPanes(xmlIn);

                    // Load DockWindows
                    if(xmlIn.Name != "DockWindows") { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                    dockWindows = LoadDockWindows(xmlIn, dockPanel);

                    // Load FloatWindows
                    if(xmlIn.Name != "FloatWindows") { throw new ArgumentException(Strings.DockPanel_LoadFromXml_InvalidXmlFormat); }
                ...
                if(contents.Length > 0) {
                    ...
                    for(int i = 0; i < contents.Length; i++) { sortedContents[i] = i; }
