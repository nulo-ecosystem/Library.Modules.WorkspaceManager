    ...
    public partial class DockWindow : Panel, INestedPanesContainer, ISplitterHost {
        ...
        Rectangle ISplitterDragSource.DragLimitBounds {
            get {
                ...
                Point location = (ModifierKeys & Keys.Shift) == 0 ? Location : DockPanel.DockArea.Location;
                if(((ISplitterDragSource)this).IsVertical) {
                    rectLimit.X += MeasurePane.MinSize;
                    rectLimit.Width -= 2 * MeasurePane.MinSize;
                    rectLimit.Y = location.Y;
                    if((ModifierKeys & Keys.Shift) == 0) { rectLimit.Height = Height; }
                } else {
                    rectLimit.Y += MeasurePane.MinSize;
                    rectLimit.Height -= 2 * MeasurePane.MinSize;
                    rectLimit.X = location.X;
                    if((ModifierKeys & Keys.Shift) == 0) { rectLimit.Width = Width; }
                }
        ...
        void ISplitterDragSource.MoveSplitter(int offset) {
            if((ModifierKeys & Keys.Shift) != 0) { SendToBack(); }
