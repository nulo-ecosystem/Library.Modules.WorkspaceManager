        ...
        private class FocusManagerImpl : Component, IContentFocusManager, IFocusManager {
            ...
            private class LocalWindowsHook : IDisposable {
                ...
                public IntPtr CoreHookProc(int code, IntPtr wParam, IntPtr lParam) {
                    if(code < 0) { return NativeMethods.CallNextHookEx(m_hHook, code, wParam, lParam); }
                ...
                // Install the hook
                public void Install() {
                    if(m_hHook != IntPtr.Zero) { Uninstall(); }
                ...
                public void Uninstall() {
                    if(m_hHook != IntPtr.Zero) {
                        _ = NativeMethods.UnhookWindowsHookEx(m_hHook);
                        m_hHook = IntPtr.Zero;
                    }
                }
            ...
            public void Activate(IDockContent content) {
                ...
                if(handler.Form.IsDisposed) { return; }
                ...
                if(handler.Form.ContainsFocus) { return; }
                if(handler.Form.SelectNextControl(handler.Form.ActiveControl, true, true, true, true)) { return; }
                if(Win32Helper.IsRunningOnMono) { return; }
            ...
            private readonly List<IDockContent> m_listContent = [];
            ...
            public void AddToList(IDockContent content) {
                if(ListContent.Contains(content) || IsInActiveList(content)) { return; }
                ListContent.Add(content);
            }

            public void RemoveFromList(IDockContent content) {
                if(IsInActiveList(content)) { RemoveFromActiveList(content); }
            ...
            private void AddLastToActiveList(IDockContent content) {
                ...
                if(last != null) { last.DockHandler.NextActive = LastActiveContent; }
            }

            private void RemoveFromActiveList(IDockContent content) {
                if(LastActiveContent == content) { LastActiveContent = content.DockHandler.PreviousActive; }
                IDockContent prev = content.DockHandler.PreviousActive;
                IDockContent next = content.DockHandler.NextActive;
                if(prev != null) { prev.DockHandler.NextActive = next; }
                if(next != null) { next.DockHandler.PreviousActive = prev; }
            ...
            public void GiveUpFocus(IDockContent content) {
                DockContentHandler handler = content.DockHandler;
                if(!handler.Form.ContainsFocus) { return; }
                if(IsFocusTrackingSuspended) { DockPanel.DummyControl.Focus(); }
            ...
            public void ResumeFocusTracking() {
                if(m_disposed || m_countSuspendFocusTracking == 0) { return; }
                if(--m_countSuspendFocusTracking == 0) {
            ...
            private DockPane GetPaneFromHandle(IntPtr hWnd) {
                ...
                for(; control != null; control = control.Parent) {
                    ...
                    if(content != null) { content.DockHandler.ActiveWindowHandle = hWnd; }
                    if(content != null && content.DockHandler.DockPanel == DockPanel) { return content.DockHandler.Pane; }
                    pane = control as DockPane;
                    if(pane != null && pane.DockPanel == DockPanel) { break; }
                }
                return pane;
            }
            ...
            private void RefreshActiveWindow() {
                ...
                if(oldActiveContent != ActiveContent) { DockPanel.OnActiveContentChanged(EventArgs.Empty); }
                if(oldActiveDocument != ActiveDocument) { DockPanel.OnActiveDocumentChanged(EventArgs.Empty); }
                if(oldActivePane != ActivePane) { DockPanel.OnActivePaneChanged(EventArgs.Empty); }
            }
            ...
            internal void SetActiveContent() {
                ...
                if(m_activeContent == value) { return; }
                if(m_activeContent != null) { m_activeContent.DockHandler.IsActivated = false; }
            ...
            private void SetActiveDocumentPane() {
                DockPane value = null;
                if(ActivePane != null && ActivePane.DockState == DockState.Document) { value = ActivePane; }
                ...
                if(m_activeDocumentPane == value) { return; }
