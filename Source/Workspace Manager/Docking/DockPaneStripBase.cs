namespace Nulo.Modules.WorkspaceManager.Docking {
    ...
    public abstract class DockPaneStripBase : Control {
        ...
        protected sealed class TabCollection : IEnumerable<Tab> {
            ...
            public Tab this[int index] {
                get {
                    ...
                    return content == null ? throw (new ArgumentOutOfRangeException(nameof(index))) : content.DockHandler.GetTab(DockPane.TabStripControl);
                }
            }
            ...
            public int IndexOf(Tab tab) {
                return tab == null ? -1 : DockPane.DisplayingContents.IndexOf(tab.Content);
            }
        ...
        protected virtual bool MouseDownActivateTest(MouseEventArgs e) => true;

        public abstract GraphicsPath GetOutline(int index);

        protected internal virtual Tab CreateTab(IDockContent content) => new Tab(content);
            ...
            if(index != -1) {
                if(e.Button == MouseButtons.Middle) {
                    ...
                    if(DockPane.ActiveContent != content) {
                        ...
                        if(MouseDownActivateTest(e)) { DockPane.ActiveContent = content; }
                    }
                }
            }
        ...
        protected override void OnMouseMove(MouseEventArgs e) {
            ...
            if(DockPane.ActiveContent == null) { return; }
            if(DockPane.DockPanel.AllowEndUserDocking && DockPane.AllowDockDragAndDrop && DockPane.ActiveContent.DockHandler.AllowEndUserDocking) { DockPane.DockPanel.BeginDrag(DockPane.ActiveContent.DockHandler); }
        }
        ...
        protected bool TryCloseTab(int index) {
            if(index >= 0 || index < Tabs.Count) {
                ...
                if(PatchController.EnableSelectClosestOnClose == true) { SelectClosestPane(index); }
                return true;
            }
            return false;
        }
        ...
        private void SelectClosestPane(int index) {
            if(index > 0 && DockPane.DockPanel.DocumentStyle == DocumentStyle.DockingWindow) {
                index--;
                if(index >= 0 || index < Tabs.Count) { DockPane.ActiveContent = Tabs[index].Content; }
            }
        }
        ...
        protected override void WndProc(ref Message m) {
            if(m.Msg == (int)Win32.Msgs.WM_LBUTTONDBLCLK) {
                ...
                if(DockPane.DockPanel.AllowEndUserDocking && index != -1) {
                    IDockContent content = Tabs[index].Content;
                    if(content.DockHandler.CheckDockState(!content.DockHandler.IsFloat) != DockState.Unknown) { content.DockHandler.IsFloat = !content.DockHandler.IsFloat; }
                }
                return;
            }
            base.WndProc(ref m);
        ...
        protected override void OnDragOver(DragEventArgs drgevent) {
            ...
            if(index != -1) {
                IDockContent content = Tabs[index].Content;
                if(DockPane.ActiveContent != content) { DockPane.ActiveContent = content; }
            }
        }
        ...
        internal static Rectangle ToScreen(Rectangle rectangle, Control parent) {
            return parent == null ? rectangle : new Rectangle(parent.PointToScreen(new Point(rectangle.Left, rectangle.Top)), new Size(rectangle.Width, rectangle.Height));
        }
